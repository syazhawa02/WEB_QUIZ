...
$arahan_cari="select*from murid where nokp_murid='".$_SESSION['nokp_murid']."' ";
...
while ($data=mysqli_fetch_array($laksana))
{
    ...
    <td class='w3-center' align='center'>".$data['bil_soalan']."</td> ";
        ...
        if ($bil_jawapan<=0)
        {
            ...
            echo "<td><a class='w3-block w3-button w3-khaki' href='arahan_latihan.php?no_set=".$data['no_set']."'>Pilih set </a></td>";
        }
        else
        {
            # jika tidak, murid hanya boleh mengulangkaji semula soalan yang telah dijawap
            echo"<td><a class='w3-block w3-button w3-khaki' href='ulangkaji.php?no_set=".$data['no_set']."&topik=".$data['topik']."&kumpul=".$kumpul."'>Ulangkaji</a></td>";
        }
